...
from ui.theme.terminal import inject_report_theme
from ui.components.terminal import term_header
...
def render_report_tab(*, ticker_a, ticker_b, a_snap, b_snap, opt_a, opt_b, cat_a, cat_b, trend_a, trend_b, regime_a, regime_b, risk_profile, lookback_days, max_news, pair, size_a_df, size_b_df):
    if st.session_state.get("report_markdown"):
        # Terminal theme (scoped)
        inject_report_theme()
        st.markdown('<div class="term-report">', unsafe_allow_html=True)
        z_hint = None
        try:
            if pair and getattr(pair, "window_used", 0) > 0 and getattr(pair, "spread_zscore", None) is not None:
                z_hint = f"Spread z {pair.spread_zscore:.2f}"
        except Exception:
            z_hint = None
        term_header(
            ticker_a=ticker_a,
        ...
        try:
            ...
            # Ensure size tables exist; if not provided from Sizing tab, compute here
            if size_a_df is None or getattr(size_a_df, "empty", False):
                size_a_df = sizing_summary_table(
                    ticker=ticker_a,
                    account_equity=float(st.session_state.get("account_equity", 0)),
                    profile_name=risk_profile,
                    spot=a_snap.get("price"),
                    vol20_ann_pct=a_snap.get("vol20_ann_pct"),
                    opt=opt_a,
                )
            if size_b_df is None or getattr(size_b_df, "empty", False):
            ...
            with actions:
                st.markdown('<div class="term-panel term-actions">', unsafe_allow_html=True)
        ...
        # Narrative (original single-column flow)
        st.markdown('<div class="term-panel">', unsafe_allow_html=True)
        st.markdown(st.session_state["report_markdown"])
        st.markdown('</div>', unsafe_allow_html=True)

        # Compact Options Snapshot table directly below narrative (single source of truth)
        st.markdown('<div class="term-panel">', unsafe_allow_html=True)
        ...
        # Sizing notes panel (single)
        st.markdown('<div class="term-panel">', unsafe_allow_html=True)
        st.markdown("#### Sizing Notes")
        st.markdown(sizing_notes_md)
        st.markdown('</div>', unsafe_allow_html=True)

        # Event Playbook section (keep original two columns A/B only)
        st.markdown("---")
        ...
        # (Event Playbook already rendered above in two columns)
